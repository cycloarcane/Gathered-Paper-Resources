...
- **R-CoT: Reverse Chain-of-Thought Problem Generation for Geometric Reasoning in Large Multimodal Models** [paper](https://paperswithcode.com/paper/r-cot-reverse-chain-of-thought-problem)  
        ...
        **Summary**: This paper introduces R-CoT, a two-stage Reverse Chain-of-Thought geometry problem generation pipeline designed to enhance the geometric reasoning capabilities of Large Multimodal Models (LMMs). The first stage, GeoChain, generates high-fidelity geometric images accompanied by descriptions that emphasize relationships among geometric elements. The second stage, Reverse A&Q, employs step-by-step reasoning based on these descriptions to generate questions in reverse from the reasoning outcomes. Experiments demonstrate that R-CoT significantly improves performance across multiple LMM baselines, achieving new records in 2B, 7B, and 8B model settings. Notably, R-CoT-8B outperforms previous state-of-the-art open-source mathematical models by 16.6% on MathVista and 9.2% on GeoQA, and surpasses the closed-source model GPT-4o by an average of 13% across both datasets.
